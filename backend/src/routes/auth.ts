...
router.post('/login', async (req: Request, res: Response) => {
  ...
  if (!senha || !verifyPassword(senha, user.passwordHash)) return res.status(401).json({ error: 'Senha incorreta.' });
  const token = signToken({ uid: user.id });
  return res.json({ ok: true, token, user: { id: user.id, nome: user.nome, email: user.email, createdAt: user.createdAt } });
});
...
router.post('/quick-token', requireAuth, async (req: Request, res: Response) => {
